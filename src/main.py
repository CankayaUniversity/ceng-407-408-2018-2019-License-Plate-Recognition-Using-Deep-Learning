...
model_unet = load_model('unet_model.h5',custom_objects={'IOU_calc_loss': IOU_calc_loss, 'IOU_calc': IOU_calc})
...
model_cnn.load_weights('cnn_model.h5')
...
#plaka resmini alıp 
def getPlateString(plate,filepath):
	...
	for cnt in contours:
		...
		if w > 15 and h > 30 and w <100 and h< 100:
			...
			cv2.imwrite(filepath + plate_char +".jpg", savedImage)
			cv2.waitKey(100)
...
def upload_file():
    ...
    filepath = './static/images/' + str(st) + 'a/'
    os.mkdir(filepath)
    f = request.files['file']
    orgfilename = filepath + 'original.jpg'
    locfilename = filepath + 'plate.jpg'
    f.save(orgfilename)
    global graph
    with graph.as_default():
        plate = getPlateImage(orgfilename)
        cv2.imwrite(locfilename,plate)
        plateString = getPlateString(plate,filepath)
        charimages = plateString
    if 'X' in plateString: plateString.remove('X')

    a = {'filepath': filepath, "letters": plateString, "char_images" : charimages }
    return json.dumps(a)
